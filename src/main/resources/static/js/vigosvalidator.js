...
const checkPassword = (pass,rules,modal,modalbody) => {
    ...
    const min = (pass, length) => pass.length >= length
    const max = (pass, length) => pass.length <= length

    const special = ['!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '_', '-', '=', '+', ',', '<', '>', '?', '"', '\'', '[', ']', '{', '}', '|', '~', '`', '\\', '/']
    const hasSpecial = (pass, count) => pass.toString().split('').filter(c => special.includes(c)).length >= count

    const isNum = (n) => !isNaN(parseFloat(n)) && isFinite(n)
    const hasNumber = (pass, count) => pass.toString().split('').filter(c => isNum(c)).length >= count

    let errors = []

    if('min' in rules){
        if(!min(password,rules.min)){
            errors.push("Your password must be at least " + rules.min + " characters long.")
        }
    }

    if('max' in rules){
        if(!max(password,rules.max)){
            errors.push("Your password must be at most " + rules.max + " characters long.")
        }
    }

    if('hasSpecial' in rules){
        if(!hasSpecial(password,rules.hasSpecial)){
            errors.push("Your password must have at least " + rules.hasSpecial + " special characters.")
        }
    }

    if('hasNumber' in rules){
        if(!hasNumber(password,rules.hasNumber)){
...
const checkUsername = (username,modalbody,modal) => {
    let errors = []
    if(Object.keys(bad_words).includes(password)){
